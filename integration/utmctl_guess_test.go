...
func TestGuessUTMLinuxVMIdentifierFromDisk(t *testing.T) {
	tmp := t.TempDir()
	oldHome := os.Getenv("HOME")
	t.Cleanup(func() { _ = os.Setenv("HOME", oldHome) })
	if err := os.Setenv("HOME", tmp); err != nil {
	...
	// Put multiple candidates; should prefer ci-Linux.
	if err := os.MkdirAll(filepath.Join(base, "ci-linux.utm"), 0o755); err != nil {
		t.Fatalf("mkdir: %v", err)
	}
	if err := os.MkdirAll(filepath.Join(base, "ci-Linux.utm"), 0o755); err != nil {
		t.Fatalf("mkdir: %v", err)
	...
	got := guessUTMLinuxVMIdentifierFromDisk()
	if got != "ci-Linux" {
		t.Fatalf("unexpected: %q", got)
	}
}
